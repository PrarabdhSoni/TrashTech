...
app.patch('/reports/:reportId/solve', async (req,res) =>{
  const reportId = parseInt(req.params.reportId, 10);
  try {
    
    await db.query('UPDATE reports SET status = $1 WHERE reportid = $2', ["Solved", reportId]);
    console.log("i am in")
  } catch (error) {
...
app.post('/reports/:userId/submit', async (req,res) =>{
  const userId = parseInt(req.params.userId, 10);
  const {bookingId, responseMessage} = req.body
  try {

    // Update the user's is_active status in the database
   await db.query('INSERT INTO reports (status, bookingid, user_id, message) VALUES ($1, $2, $3, $4)', ["In process", bookingId, userId, responseMessage]);
...
app.get('/bookings/:userId/id', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'SELECT id FROM bookings WHERE user_id = $1 ORDER BY created_at DESC',
      [userId]
    );
    res.json(result);
  } catch (error) {
...
app.post('/commercial/api/book', async (req, res) => {
  const {
    name, 
    mobile_number, 
    address, 
  ...
  try {
    ...
    await db.query(
      ...
      (name, mobile_no, address, state, city, type_of_waste, interval, amount, userid, paid) 
...
app.get('/comercial/Abookings', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM comercial ORDER BY created_at DESC');
    res.json(result);
  } catch (error) {
    console.error('Error fetching unavailable slots:', error.message);
    res.status(500).send('Server error');
...
app.put('/api/users/:userId', async (req, res) => {
  ...
  try {
    ...
    if (typeof is_active !== "boolean") {
      return res.status(400).json({ error: "Invalid input for is_active. Expected a boolean." });
    }

    // Update the user's is_active status in the database
    const result = await db.query('UPDATE users SET is_active = $1 WHERE id = $2 RETURNING *', [is_active, userId]);
...
app.put('/api/allEmployee/:userId', async (req, res) => {
  console.log("hi");
  ...
  try {
    ...
    // Update the user's is_active status in the database
    const result = await db.query('UPDATE employee SET is_active = $1 WHERE employee_id = $2 RETURNING *', [is_active, userId]);
...
app.get('/Abookings/Reports', async (req, res) => {
  try {
